...
QR_dev_path = "data/QR/QR_new.csv"

embedding_path = "glove.twitter.27B.50d.txt"
word_dict_path = "word_dict.txt"
log_dir = "logs"
preTrained_model = "model/preTrained_model.ckpt"
...
k = 32
batch_size = 128
preTraining_epochs = 1 # pre-training epochs --> Auxiliary Task (QA)
fineTune_epochs = 50 # fine-tune iterations--> Main task iteration (QR)
...
with tf.Session() as sess:
    ...
    for m in range(fineTune_epochs):
        ...
        for i in range(len(Y_batch)):
            ...
            print("Fine-tune epoch:", m + 1, "--iter:", i + 1, "/", len(Y_batch), "--loss:", Loss_main)
            step += 1
        # Validation
        ...
        print("[Valid]: --precision: ", precision, "--recall: ", recall, "--f1: ", f1)
